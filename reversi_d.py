...
def cpu_move(board, player_num):
    ...
    x_pos = [(2,1), (7,1), (1,2), (2,2), (7,2), (8,2), (1,7), (2,7), (7,7), (7,8), (2,8), (8,7)]
    edge = [(1,1), (8,8), (1,8), (8,1)]
    waki = [(1,3), (1,4), (1,5), (1,6), (3,1), (3,8), (4,1), (4,8), (5,1), (5,8), (6,1), (6,8), (8,3), (8,4), (8,5), (8,6)]

    reversible_without_edge_x = []
    reversible_edge = []
    reversible_waki = []
    reversible_x = []
    reversible_x_igai = []

    valid_length = int(len(valid_moves))
    print(valid_length)

    for i in range(valid_length):
        if valid_moves[i] in edge:
            reversible_edge.append(valid_moves[i]) #取れる角のリスト作成
        else:
            reversible_without_edge_x.append(valid_moves[i])

    for i in range(valid_length):
        if valid_moves[i] in waki:
            reversible_waki.append(valid_moves[i]) #取れる側のリスト作成
        else:
            reversible_without_edge_x.append(valid_moves[i])
    
    for i in range(valid_length):
        if valid_moves[i] in x_pos:
            reversible_x.append(valid_moves[i]) #取れるxのリスト作成
        else:
            reversible_x_igai.append(valid_moves[i])

    if reversible_edge != []:
        return reversible_edge[0]
    elif reversible_waki != []:

        return reversible_waki[0]
    elif reversible_x_igai != []:
        #for i in range(valid_length):
            #if valid_moves[i] in x_pos:
                #valid_moves.remove(valid_moves[i])
        #valid_moves = list(filter(lambda move: move not in x_pos, valid_moves))
        ...
        for move in valid_moves:
            temp_board = put_disc(board, player_num, move[0], move[1])
            hikkuris = []
            for x in range(1, 9):
                for y in range(1, 9):
                    if temp_board[y][x] != board[y][x]:
                        hikkuris.append([x, y])
            hikkuris.remove(list(move))
            kaihoudo = 0
            for hikkuri in hikkuris:
                for dx in [-1, 0, 1]:
                    for dy in [-1, 0, 1]:
                        if temp_board[hikkuri[0]-dy][hikkuri[1]-dx] == 0:
                            kaihoudo += 1
            kaihou_move.append([move, kaihoudo])
        min = 1000
        best_move = None
        for move in kaihou_move:
            if move[1] < min:
                min = move[1]
                best_move = move[0]
            print(move)
