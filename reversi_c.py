...
def cpu_move_prioriry_avoidcustomized(board, player_num):
    valid_moves = []
    corners = []
    safe_edges = []
    priority3 = []
    ...
    # 危険な手（角の隣接セル）の定義
    dangerous_list = [
        (1,2), (2,1), (2,2),    # 左上角 (1,1) の周辺
        (7,1), (7,2), (8,2),    # 右上角 (8,1) の周辺
        (1,7), (2,7), (2,8),    # 左下角 (1,8) の周辺
        (7,8), (7,7), (8,7)    # 右下角 (8,8) の周辺
    ]
        ...
        for y in range(1, 9):
            if validate_reversible(board, player_num, x, y):
                ...
                if (x, y) in [(1,1), (1,8), (8,1), (8,8)]:
                    corners.append((x, y))
                # 辺の手（角は除く）
                elif (x,y) in [(1,3),(1,4),(1,5),(1,6),
                               (8,3),(8,4),(8,5),(8,6),
                               (3,1),(4,1),(5,1),(6,1),
                               (3,8),(4,8),(5,8),(6,8),]:
                    safe_edges.append((x, y))
                # 優先順位3位
                elif (x,y) in [(3,3),(3,6),(6,3),(6,6)]:
                    priority3.append((x,y))
                # 危険かどうかで振り分け
                elif (x, y) not in dangerous_list:
                    if (x, y) not in dangerous_list2:
                        safe_moves.append((x, y))
                    else:
                        lessdangerousMoves.append((x,y))
                
                
                    
    # 安全な辺の手
    #safe_edges = [move for move in edges if move in safe_moves]
    
    if corners != []:
        #return random.choice(corners)
        bestmove = cpuCheck(board, player_num,corners)
        return bestmove
    elif safe_edges != []:
        #return random.choice(safe_edges)
        bestmove = cpuCheck(board, player_num,safe_edges)
        return bestmove
    elif priority3 != []:
        #return random.choice(priority3)
        bestmove = cpuCheck(board, player_num,priority3)
        return bestmove
    elif safe_moves != []:
        #return random.choice(safe_moves)
        bestmove = cpuCheck(board, player_num,safe_moves)
        return bestmove
    elif lessdangerousMoves != []:
        #return random.choice(lessdangerousMoves)
        bestmove = cpuCheck(board, player_num,lessdangerousMoves)
        return bestmove
    elif valid_moves != []:
        #return random.choice(valid_moves)
        bestmove = cpuCheck(board, player_num,valid_moves)
        return bestmove
    else:
        #return valid_moves
        bestmove = cpuCheck(board, player_num,valid_moves)
...
def cpuCheck(board, player_num, validMoves):
    def get_my_count(board, player_num):
        return count_discs(board)[0] if player_num == 1 else count_discs(board)[1]

    best_move, best_score = None, -1000
    
    for x, y in validMoves:
        if not validate_reversible(board, player_num, x, y):
            continue
        
        board_after_my_move = put_disc(board, player_num, x, y)
        opponent = rival_player_num(player_num)
        opponent_moves = [(ox, oy) for ox in range(1, 9) for oy in range(1, 9) 
                          if validate_reversible(board_after_my_move, opponent, ox, oy)]
        
        worst_outcome = min(
            (max(get_my_count(put_disc(board_after_my_move, player_num, nx, ny), player_num) 
                 for nx, ny in [(nx, ny) for nx in range(1, 9) for ny in range(1, 9) 
                                 if validate_reversible(board_after_my_move, player_num, nx, ny)])
             if any(validate_reversible(board_after_my_move, player_num, nx, ny) 
                    for nx in range(1, 9) for ny in range(1, 9))
             else get_my_count(board_after_my_move, player_num))
            for ox, oy in opponent_moves) if opponent_moves else get_my_count(board_after_my_move, player_num)
        
        if worst_outcome > best_score:
            best_score, best_move = worst_outcome, (x, y)
...
class ReversiGUI:
    ...
    def cpu_turn(self):
        ...
        if (self.player_num == 1 and self.cpu_first) or (self.player_num == -1 and self.cpu_second):
            # この関数を変更
            move = cpu_move_prioriry_avoidcustomized(self.board, self.player_num)
            if move != []:
                x, y = move
